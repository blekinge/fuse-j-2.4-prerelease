...
public class CAPIGenerator {
	...
	public void generateClassAPI( PrintWriter hOut, PrintWriter cOut, boolean extraArgForInterfaces, boolean includeStaticFields ) {
		...
		for( int i = 0; i < constructors.length; i++ ) {
			...
			if( Modifier.isPublic( constructor.getModifiers() ) ) {
				...
				constructor2name.put( constructor, getMethodName( "new", constructor.getParameterTypes(), Void.TYPE ) );
			}
		}
      ...
      // split all public methods into static and instance methods and give them C names
		...
		for( int i = 0; i < methods.length; i++ ) {
			...
			if( Modifier.isPublic( mod ) ) {
				...
				method2name.put( method, getMethodName( method.getName(), method.getParameterTypes(), method.getReturnType() ) );
			}
		}
		...
		if( staticFields.length > 0 && includeStaticFields ) {
			...
			for( int i = 0; i < staticFields.length; i++ ) {
				Field field = staticFields[ i ];
				cOut.print( "      " + structVarName + "->static_field." + field.getName() + " = (*env)->GetStaticFieldID(env, " + structVarName + "->class, \"" + field.getName() + "\", \"" + getJVMTypeSignature( field.getType() ) + "\");\n" + "      if ((*env)->ExceptionCheck(env)) break;\n" );
			}
		...
		if( instanceFields.length > 0 ) {
			...
			for( int i = 0; i < instanceFields.length; i++ ) {
				Field field = instanceFields[ i ];
				cOut.print( "      " + structVarName + "->field." + field.getName() + " = (*env)->GetFieldID(env, " + structVarName + "->class, \"" + field.getName() + "\", \"" + getJVMTypeSignature( field.getType() ) + "\");\n" + "      if ((*env)->ExceptionCheck(env)) break;\n" );
			}
		...
		if( constructors.length > 0 ) {
			...
			for( int i = 0; i < constructors.length; i++ ) {
				Constructor constructor = constructors[ i ];
				cOut.print( "      " + structVarName + "->constructor." + constructor2name.get( constructor ) + " = (*env)->GetMethodID(env, " + structVarName + "->class, \"" + "<init>" + "\", \"" + getJVMConstructorSignature( constructor ) + "\");\n" + "      if ((*env)->ExceptionCheck(env)) break;\n" );
			}
		...
		if( staticMethods.length > 0 ) {
			...
			for( int i = 0; i < staticMethods.length; i++ ) {
				Method method = staticMethods[ i ];
				cOut.print( "      " + structVarName + "->static_method." + method2name.get( method ) + " = (*env)->GetStaticMethodID(env, " + structVarName + "->class, \"" + method.getName() + "\", \"" + getJVMMethodSignature( method ) + "\");\n" + "      if ((*env)->ExceptionCheck(env)) break;\n" );
			}
		...
		if( instanceMethods.length > 0 ) {
			...
			for( int i = 0; i < instanceMethods.length; i++ ) {
				Method method = instanceMethods[ i ];
				cOut.print( "      " + structVarName + "->method." + method2name.get( method ) + " = (*env)->GetMethodID(env, " + structVarName + "->class, \"" + method.getName() + "\", \"" + getJVMMethodSignature( method ) + "\");\n" + "      if ((*env)->ExceptionCheck(env)) break;\n" );
			}
	...
	private String getMethodName( String methodName, Class[] argumentTypes, Class returnType ) {
		StringBuffer buf = new StringBuffer( methodName );
		if( argumentTypes != null && argumentTypes.length > 0 ) {
			buf.append( "__" );
			appendJVMTypeSignatures( argumentTypes, buf );
		}
		buf.append( "__" );
		appendJVMTypeSignature( returnType, buf );
		return mangle( buf.toString() );
	}
}
